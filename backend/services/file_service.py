...
class FileService:
    ...
    def __init__(self, input_dir: str = "input") -> None:
        ...
        logger.info(
            "FileService 已初始化（带扫描缓存）",
            extra={"stage": "file_service_init", "scan_ttl": self._cache._ttl},
        )
    ...
    def scan(self) -> list[dict[str, Any]]:
        ...
        if cached is not None:
            logger.debug(
                "scan 命中缓存: %d 个文件",
                len(cached),
                extra={"stage": "file_scan_cache_hit", "count": len(cached)},
            )
            return cached
        logger.info(
            "扫描输入目录: %s",
        ...
        for tf in tables:
            ...
            has_rotated = (
                next(self._output_dir.glob(f"{outcrop}_rotated(strike=*).png"), None) is not None
            )
