...
- [THIRD_PARTY_NOTICES.txt](file://THIRD_PARTY_NOTICES.txt)
</cite>

## 更新摘要
**变更内容**   
- 新增第三方许可合规系统，包含完整的 THIRD_PARTY_NOTICES.txt 文件
- 增强打包脚本的法律文件验证和复制功能
- 改进构建流程中的法律声明检查机制
- 完善所有分发产物的法律文档集成
...
- **第三方许可合规管理**（许可证文件验证、法律声明集成、合规性检查）
...
- **法律声明与合规**
  - LICENSE：项目主许可证文件
  - THIRD_PARTY_NOTICES.txt：第三方组件许可声明和合规信息
- 安装器与产物
...
K["LICENSE<br/>项目许可证"] --> L["法律声明验证"]
M["THIRD_PARTY_NOTICES.txt<br/>第三方许可声明"] --> L
L --> N["copy_legal_files()<br/>复制到分发目录"]
N --> E
N --> F
N --> G
```
...
**图表来源**
- [scripts/package.py:36-40](file://scripts/package.py#L36-L40)
- [scripts/package.py:147-159](file://scripts/package.py#L147-L159)
- [scripts/package.py:252-258](file://scripts/package.py#L252-L258)
- [TracePipeline.spec:31-33](file://TracePipeline.spec#L31-L33)
...
- [scripts/package.py:252-258](file://scripts/package.py#L252-L258)
- [TracePipeline.spec:31-33](file://TracePipeline.spec#L31-L33)
- [THIRD_PARTY_NOTICES.txt:1-116](file://THIRD_PARTY_NOTICES.txt#L1-L116)
...
- 一键打包脚本（scripts/package.py）
  ...
  - **法律声明验证：确保 LICENSE 和 THIRD_PARTY_NOTICES.txt 文件存在**
  - **法律声明复制：copy_legal_files() 函数将法律文件复制到分发目录**
...
Pkg->>Legal : 检查 LICENSE 和 THIRD_PARTY_NOTICES.txt
Legal-->>Pkg : 验证结果
alt 法律声明缺失
Pkg-->>Dev : 报错并退出
else 法律声明完整
...
Pkg->>Legal : copy_legal_files() 复制法律文件
Legal-->>Pkg : 确认法律文件已复制
alt 生成安装版
Pkg->>ISS : 编译 .iss 安装脚本
...
### 一键打包脚本（scripts/package.py）
- 前置检查
  - venv Python、PyInstaller、图标、**法律声明（LICENSE 和 THIRD_PARTY_NOTICES.txt）**、Inno Setup、7-Zip（含 7z.sfx）
- 版本读取
...
- PyInstaller 打包
  - 调用 .venv/Scripts/pyinstaller.exe 执行 TracePipeline.spec，清理旧 dist 与临时文件，**使用 copy_legal_files() 复制法律声明到程序目录**
- Inno Setup 安装程序
  - 生成 .iss 脚本（语言选择、文件/图标/任务/运行/卸载删除等），调用 ISCC.exe 编译
- 7-Zip 便携版
  - 使用 mx=9/md=256m 最高压缩比创建 .7z，拼接 7z.sfx 与配置文件生成自解压 exe，并清理临时文件
- **法律声明管理**
  - LEGAL_FILES 常量定义需要包含的法律文件列表
  - copy_legal_files() 函数负责将法律文件复制到目标目录
  - 在打包前验证法律文件存在性，缺失则阻止打包流程
...
**图表来源**
- [scripts/package.py:210-563](file://scripts/package.py#L210-563)
- [scripts/package.py:147-159](file://scripts/package.py#L147-159)
...
- **法律声明文件结构**
  - LICENSE：项目主许可证（MIT License）
  - THIRD_PARTY_NOTICES.txt：详细的第三方组件许可声明
- **验证机制**
  - 打包前检查：确保 LICENSE 和 THIRD_PARTY_NOTICES.txt 存在
  - 构建阻断：缺少任何法律文件将阻止打包流程
- **复制策略**
  - copy_legal_files() 函数将法律文件复制到所有分发产物
  - 支持程序文件夹、安装程序、便携版等多种分发格式
- **合规内容**
  - 运行时组件许可声明（Python、NumPy、pandas、Matplotlib 等）
  - 前端运行时组件（Vue、Element Plus、ECharts 等）
  - 打包工具组件（PyInstaller、Inno Setup、7-Zip 等）
  - 弱 copyleft 组件源码可用性说明
  - Microsoft WebView2 Runtime 使用说明

```mermaid
flowchart TD
A["法律声明文件"] --> B["LICENSE"]
A --> C["THIRD_PARTY_NOTICES.txt"]
B --> D["打包前验证"]
C --> D
D --> E{"文件存在?"}
E -- 否 --> F["错误: 法律声明缺失"]
E -- 是 --> G["copy_legal_files() 复制"]
G --> H["复制到程序文件夹"]
G --> I["包含在安装程序中"]
G --> J["随附便携版"]
H --> K["合规分发"]
...
- 脚本生成 .iss 内容
  ...
  - **法律声明文件：自动包含 LICENSE 和 THIRD_PARTY_NOTICES.txt**
...
F --> G["附带 THIRD_PARTY_NOTICES.txt"]
```
...
- 常见打包错误
  ...
  - **法律声明缺失：LICENSE 与 THIRD_PARTY_NOTICES.txt 必须存在，否则打包将被阻止**
- 工具链问题
...
- **法律声明相关问题**
  - **THIRD_PARTY_NOTICES.txt 内容不完整：确保包含所有第三方组件的许可信息**
  - **法律文件格式错误：遵循标准格式，包含组件名称、许可证类型、版权信息等**
  - **合规性检查失败：检查是否有新的第三方依赖未添加到法律声明文件中**
...
## 结论
TracePipeline 的打包与分发采用"前端构建 + PyInstaller + Inno Setup + 7-Zip SFX"的一体化流水线，配合统一的版本源与依赖声明，实现了安装版与便携版的稳定产出。**新增的第三方许可合规系统确保了法律文件的完整性与合规性，通过严格的验证机制防止缺少必要法律声明的分发产物**。通过合理的隐藏导入、数据文件与图标配置，以及严格的运行时依赖检测（如 WebView2），确保了用户在 Windows 平台的开箱即用体验。建议在生产环境中固化工具链路径与环境变量，并结合 RELEASE_NOTES.md 进行规范的版本发布。
...
- **法律声明文件**
  - **项目许可证：LICENSE**
  - **第三方许可声明：THIRD_PARTY_NOTICES.txt**
  - **分发产物中的法律文件：所有分发版本都包含完整的法律声明**
