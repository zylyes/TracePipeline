...
def format_statistics_box_lines(stats: TraceStatistics) -> tuple[str, ...]:
    ...
    return (
        ...
        f"I/II/III: {stats.type_i_count}/{stats.type_ii_count}/{stats.type_iii_count}",
        f"L_hat: {_format_value(stats.scanline_length_estimate, ' m')}",
        f"L_obs_all: {_format_value(stats.observed_mean_length_all, ' m')}",
        f"L_obs_I: {_format_value(stats.observed_mean_length_i, ' m')}",
        f"L_est: {_format_value(stats.estimated_mean_trace_length, ' m')}",
        f"P10_I: {_format_value(stats.p10_i, ' 1/m')}",
        f"P10_all: {_format_value(stats.p10_all, ' 1/m')}",
        f"P20: {_format_value(stats.p20, ' 1/m^2')}",
        f"P21_est: {_format_value(stats.p21_est, ' 1/m')}",
        f"P21_obs: {_format_value(stats.p21_obs, ' 1/m')}",
    )
