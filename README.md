...
| **迹线统计指标** | I/II/III 型自动分类，P10/P20/P21 密度参数（实测优先 → 圆窗闭式 → 凸包面积三级回退），圆形取样窗法 4 策略自适应（tangent/hybrid/concentric + auto 6因子加权评分），Mauldon 平均迹长估计 |
...
| 指标 | 含义 | 计算方法 |
|------|------|----------|
| **P10** | 线密度（m⁻¹） | 迹线数 ÷ 测线长度；测线长度优先读取列 11（Excel 第 12 列）实测值，缺失/非法时由 r1 间距估算 |
...
| **露头面积** | 露头有效面积（m²） | 实测优先（列 12/Excel 第 13 列）；缺失时使用迹线端点凸包面积（Convex Hull），点数 < 3 或共线时返回 NaN
...
**auto 策略评分机制**：对 tangent/hybrid/concentric 三种候选策略分别计算 6 个加权评分因子——① 有效组数（×1.45）② 有效组比例（×1.00）③ 空间覆盖（×1.35，含双侧平衡度 + 沿测线分布）④ 指标稳定性（×1.10，P20/P21/L 变异系数倒数）⑤ 半径尺度（×1.00）⑥ 样本充足率（×1.10）。加权总分最高者胜出；若最高分与粗估密度偏好的分差 ≤ 12% 容差，回退到密度偏好策略；无有效候选时按粗估面密度降级选择（tangent → hybrid → concentric）。
...
> ⚠️ 上表统计值为占位符（`—`）。运行 `python run_trace_pipeline.py` 后终端自动输出各露头完整汇总表（含策略列），实际值以终端输出和迹线图内置 LaTeX 统计信息框为准。来源标注：(M) 实测、(W) 圆窗、(E) 估算。
...
- **统计增强**：新增 I/II/III 型自动分类、凸包露头面积、实测优先 P10/P20/P21 密度参数（三级回退管道），圆形取样窗法 4 策略自适应（auto 6因子加权评分 / tangent / hybrid / concentric）+ Mauldon 平均迹长，来源标注 (M)/(W)/(E)
...
新增模块在 `trace_pipeline/` 下创建，在 `__init__.py` 中导出。流水线编排在 `pipeline.py:run_pipeline()` 中，可按需插入或替换阶段。顶层包导出 20 个常用入口（含 `TraceStatisticsConfig`、`compute_trace_statistics` 等统计接口），底层函数可直接导入：
